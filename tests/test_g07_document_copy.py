...
def open_session(path: Path):
    history=TextHistory(); session=DocumentSession(); loaded=load_document(str(path)); state=history.reset(loaded.text); session.establish_open(loaded,state); return history,session

def edit(history,session,text):
    history.commit(text); session.commit_history_state(history.current)


class G07DocumentCopyTests(unittest.TestCase):
    def test_modified_copy_preserves_exact_session_binding_savepoint_and_history(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); src=root/"source.txt"; dst=root/"copy.txt"; src.write_text("A\n",encoding="utf-8")
            h,s=open_session(src); edit(h,s,"AB\n"); service=DocumentCopyService(session=s,writer=GuardedFileWriter())
            before=s.snapshot(); hcp=h.checkpoint(); obs=service.observe_target(str(dst)); service.copy_to(obs)
    ...
    def test_copy_preserves_utf16_bom_and_crlf_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); src=root/"source.txt"; dst=root/"copy.txt"
            src.write_bytes(codecs.BOM_UTF16_LE+"A\r\n".encode("utf-16-le"))
            h,s=open_session(src); edit(h,s,"AB\n"); service=DocumentCopyService(session=s,writer=GuardedFileWriter())
            service.copy_to(service.observe_target(str(dst)))
            raw=dst.read_bytes(); self.assertTrue(raw.startswith(codecs.BOM_UTF16_LE)); self.assertEqual(load_document(str(dst)).text,"AB\n")

    def test_mixed_eol_requires_explicit_consent(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); src=root/"mixed.txt"; dst=root/"copy.txt"; src.write_bytes(b"A\r\nB\nC\r\n")
            ...
            service.copy_to(obs,allow_mixed_eol_normalization=True); self.assertTrue(dst.exists())

    def test_active_logical_and_symlink_alias_targets_are_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); src=root/"source.txt"; alias=root/"alias.txt"; src.write_text("A",encoding="utf-8"); alias.symlink_to(src)
            h,s=open_session(src); service=DocumentCopyService(session=s,writer=GuardedFileWriter())
    ...
    def test_version_copy_uses_max_plus_one_width_and_ignores_unrelated(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            h,s=open_session(src); service=DocumentCopyService(session=s,writer=GuardedFileWriter())
            plan=service.plan_named_version_copy(); self.assertEqual(plan.number,10003); self.assertTrue(plan.logical_target_path.endswith("report_v10003.md"))
            obs=service.observe_planned_version_target(plan); service.copy_to(obs); self.assertEqual(Path(plan.logical_target_path).read_text(),"A")

    def test_version_dotfile_and_no_suffix(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td)
            for name,expected in ((".notes",".notes_v0001"),("README","README_v0001")):
                src=root/name; src.write_text("A",encoding="utf-8"); h,s=open_session(src); service=DocumentCopyService(session=s,writer=GuardedFileWriter())
    ...
    def test_version_target_race_fails_closed_not_renumbered(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td); src=root/"doc.txt"; src.write_text("A",encoding="utf-8"); h,s=open_session(src); service=DocumentCopyService(session=s,writer=GuardedFileWriter())
            plan=service.plan_named_version_copy(); Path(plan.logical_target_path).write_text("racer",encoding="utf-8")
