...
from graphium.application.text_transform import (
    MAX_TRANSFORM_CHANGED_SPANS,
    TransformInputError,
    TransformScaleError,
    TransformationPlan,
    build_transformation_plan,
    plan_duplicate_line_selection,
    plan_lowercase,
    plan_move_lines_down,
    plan_move_lines_up,
    plan_trim_trailing_spaces,
    plan_uppercase,
)
...
class G09PlannerSemanticsTests(unittest.TestCase):
    ...
    def test_06_duplicate_selection_is_inserted_after_and_selected(self):
        plan = plan_duplicate_line_selection(
            source_text="abcde", source_state_id=1, before_view=ViewState(1, 4)
        )
    ...
    def test_07_duplicate_reversed_selection_preserves_direction(self):
        plan = plan_duplicate_line_selection(
            source_text="abcde", source_state_id=1, before_view=ViewState(4, 1)
        )
        self.assertEqual(plan.final_text, "abcdbcde")
    ...
    def test_21_trim_whole_document_removes_only_space_and_tab(self):
        plan = plan_trim_trailing_spaces(
    ...
    def test_26_dispatcher_rejects_unknown_action(self):
        with self.assertRaises(TransformInputError):
            build_transformation_plan(
                "title-case", source_text="abc", source_state_id=1, before_view=ViewState()
            )
    ...
    def test_29_uppercase_expansion_that_breaks_renderer_limit_fails_before_plan_return(self):
        # Each ß becomes SS; the source is within the line limit but the result is not.
        text = "ß" * (MAX_INTERACTIVE_LINE_CHARS // 2 + 1)
        with self.assertRaises(InteractiveRenderabilityError):
            plan_uppercase(
                source_text=text, source_state_id=1,
                before_view=ViewState(len(text), 0),
            )

    def test_30_duplicate_that_breaks_renderer_limit_fails_before_plan_return(self):
        text = "x" * MAX_INTERACTIVE_LINE_CHARS
        # Selection duplication on the same line doubles it without inserting a newline.
        with self.assertRaises(InteractiveRenderabilityError):
            plan_duplicate_line_selection(
                source_text=text, source_state_id=1,
                before_view=ViewState(len(text), 0),
    ...
    def test_31_plan_is_immutable(self):
        ...
        with self.assertRaises(FrozenInstanceError):
            plan.final_text = "x"  # type: ignore[misc]


class G09ArchitectureTests(unittest.TestCase):
    def test_32_command_surface_has_exact_six_transform_actions_and_two_accelerators(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                ("duplicate-line-selection", "Duplicate Line / Selection", None),
                ("move-lines-up", "Move Lines Up", "<Alt>Up"),
                ("move-lines-down", "Move Lines Down", "<Alt>Down"),
                ("trim-trailing-spaces", "Trim Trailing Spaces", None),
            ],
        )
    ...
    def test_33_case_command_availability_tracks_nonempty_selection_only(self):
        off = command_availability(
            modified=False, has_path=True, can_undo=False, can_redo=False, has_selection=False
        )
        on = command_availability(
            modified=False, has_path=True, can_undo=False, can_redo=False, has_selection=True
        )
    ...
    def test_34_window_uses_nested_transform_menu_and_single_g05_mutation_authority(self):
        text = (ROOT / "graphium/adapters/gtk/window.py").read_text(encoding="utf-8")
        self.assertIn('section.append_submenu("Transform Text", transform_menu)', text)
        self.assertIn("apply_prevalidated_programmatic_group", text)
        self.assertNotIn('root.append_submenu("Format"', text)
        planner = (ROOT / "graphium/application/text_transform.py").read_text(encoding="utf-8")
        self.assertNotIn("gi.repository", planner)
        self.assertNotIn("GtkSourceView", planner)

    def test_35_runtime_identity_is_g09(self):
