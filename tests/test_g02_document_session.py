...
def file_state(path: str = "/tmp/example.txt") -> DocumentFileState:
    return DocumentFileState(
        ...
        load=DocumentLoadMetadata(
            "utf-8", BomKind.NONE,
            LineEndingProfile(LineEnding.LF, False, True, lf_count=1),
        ),
        disk=DiskObservation(2, 10, 0o100644, False),
        content_fingerprint=ContentFingerprint("sha256", "00" * 32),
    )


class G02DocumentSessionTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_open_is_clean_and_retains_g01_file_state(self):
        fs = file_state()
