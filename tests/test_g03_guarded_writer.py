...
class G03GuardedWriterTests(unittest.TestCase):
    def _write(self, path: Path, data: bytes, mode: int = 0o644) -> None:
    ...
    def test_observe_existing_matches_accepted_g01_baseline(self):
        with tempfile.TemporaryDirectory() as td:
    ...
    def test_existing_commit_replaces_bytes_and_preserves_mode(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(
                result.committed_fingerprint.hex_digest,
    ...
    def test_write_failure_leaves_existing_target_byte_identical(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            obs = writer.observe_target(str(target), expected_file_state=accepted)
            with patch(
                "graphium.infrastructure.guarded_file_writer.os.write",
    ...
    def test_stage_fsync_failure_leaves_existing_target_byte_identical(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            obs = writer.observe_target(str(target), expected_file_state=accepted)
            with patch(
                "graphium.infrastructure.guarded_file_writer.os.fsync",
                side_effect=OSError(errno.EIO, "sync fail"),
            ):
                with self.assertRaises(Exception):
                    writer.commit(obs, b"replacement\n")
            self.assertEqual(target.read_bytes(), b"original\n")
    ...
    def test_accessible_extended_attributes_are_preserved(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = writer.commit(
                writer.observe_target(str(target), expected_file_state=accepted),
                b"new\n",
            )
    ...
    def test_absent_target_uses_frozen_new_file_mode(self):
        with tempfile.TemporaryDirectory() as td:
            target = Path(td) / "new.txt"
            writer = GuardedFileWriter(new_file_mode=0o640)
