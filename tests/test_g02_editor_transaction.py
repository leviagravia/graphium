...
def file_state(path: str = "/tmp/open.txt") -> DocumentFileState:
    return DocumentFileState(
        binding=DocumentFileBinding(path, path, FileObjectIdentity(1, 2)),
        load=DocumentLoadMetadata(
            "utf-8", BomKind.NONE,
            LineEndingProfile(LineEnding.LF, False, False),
        ),
        disk=DiskObservation(1, 10, 0o100644, False),
        content_fingerprint=ContentFingerprint("sha256", "11" * 32),
    )


class G02EditorTransactionTests(unittest.TestCase):
    ...
    def test_branch_after_undo_with_same_saved_text_remains_dirty_by_identity(self):
        buffer, history, session, controller = self.make_controller("A")
        buffer.set("AB")
        controller.observe_native_change(); controller.commit_native_group()
    ...
    def test_failed_programmatic_transaction_rolls_back_buffer_history_and_session(self):
        buffer, history, session, controller = self.make_controller("A")
        hcp = history.checkpoint(); scp = session.snapshot(); before = buffer.capture()
        def action():
            buffer.set("BROKEN")
    ...
    def test_failed_undo_restore_rolls_back_history_session_and_visible_buffer(self):
        buffer, history, session, controller = self.make_controller("A")
        buffer.set("AB"); controller.observe_native_change(); controller.commit_native_group()
        hcp = history.checkpoint(); scp = session.snapshot(); before = buffer.capture()
    ...
    def test_failed_programmatic_rollback_restores_authorities_even_if_buffer_restore_fails(self):
        buffer, history, session, controller = self.make_controller("A")
        hcp = history.checkpoint(); scp = session.snapshot()
        def action():
            buffer.set("BROKEN")
            buffer.fail_next_restore = True
    ...
    def test_late_save_specific_state_does_not_clean_newer_current_state(self):
        ...
        buffer.set("ABC"); controller.observe_native_change(); controller.commit_native_group()
