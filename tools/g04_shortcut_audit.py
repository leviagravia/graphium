#!/usr/bin/env python3
"""Audit Graphium accelerators against active Cinnamon/GNOME global keybindings."""
...
if "--bootstrap-only" in sys.argv:
    print(f"G04_SHORTCUT_BOOTSTRAP=PASS root={ROOT}")
    raise SystemExit(0)
...
def main() -> None:
    ...
    if proc.returncode != 0:
        raise SystemExit(f"SHORTCUT_AUDIT_FAIL: gsettings rc={proc.returncode}: {proc.stderr.strip()}")
    ...
    if collisions:
        for action, accel, lines in collisions:
            ...
            for line in lines[:5]:
                print(f"  {line}")
        raise SystemExit("SHORTCUT_COLLISION_GATE=FAIL")

    known = normalize("<Ctrl><Alt>L")
    print(f"KNOWN_MINT_RESERVED_CTRL_ALT_L_PRESENT={'YES' if known in active else 'NOT_OBSERVED'}")
    print("SHORTCUT_COLLISION_GATE=PASS")
    print("FINAL_PHASE=G04_SHORTCUT_AUDIT_PASS")
